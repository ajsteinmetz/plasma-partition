...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\textwidth]{ChemicalPotential_04.jpg}
...
This feature is plotted in \rf{fig:momentperlepton} where the average magnetic moment $\vert\vec{m}\vert_{z}$ defined in \req{momentperlepton} displays how essential the external field is on the \lq per lepton\rq\ magnetization. Both the $b_{0}=10^{-11}$ (lower plot, red curve) and $b_{0}=10^{-3}$ (upper plot, blue curve) cosmic magnetic scale bounds are plotted in the Boltzmann approximation. The constant magnetic field case (solid green) with the comoving value chosen at temperature $T_{0}=10\keV$. The average magnetic moment per lepton is suppressed at higher temperatures (as expected for magnetization satisfying Curie's law) if the field strength is held constant. This demonstrates the importance of the conservation of primordial magnetic flux in the plasma, required by \req{bscale}, through comoving surfaces for the external field ensures that as we travel into the past, the magnetic field increases which enhances the overall magnetization of the gas. 
...
\begin{align}
    \lim_{b_{0}\rightarrow0}{\mathfrak M}=\frac{q^{2}}{\pi^{2}}\frac{T^{2}}{m_{e}^{2}}\sinh{\frac{\eta}{T}}\cosh{\frac{\mu}{T}}\left[\frac{m_{e}}{T}K_{1}\left(\frac{m_{e}}{T}\right)\right]\,,\qquad2\sinh{\frac{\eta}{T}}=\xi-\xi^{-1}\,.
\end{align}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\textwidth]{SpinPolarizationEffect_02.jpg}
