...
\begin{figure}[ht]
    ...
    \caption{The magnetization ${\cal M}/{\cal H}_{C}$ of the primordial $e^{+}e^{-}$ plasma is plotted as a function of temperature. The lower (solid red) and upper (solid blue) bounds for cosmic magnetic scale $b_{0}$ are included. The external magnetic field $B/H_{C}$ is also plotted in for lower (dashed red) and upper (dashed blue) bounds. The spin fugacity is set to unity $\xi=1$. {\xred Andrew - Have this plot replaced with non-zero chemical potential. Caption written with dimensionless units in mind, but alternative in Gauss is also an option.}}
    \label{magnetization} 
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
