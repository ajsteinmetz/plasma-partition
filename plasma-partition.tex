...
We do not address here the origin of the spin polarization phenomena but recall that the $e^{+}e^{-}$ pair plasma decouples from the neutrino background near to $T=2000\keV$~\cite{Birrell:2014uka}. Therefore
...
\begin{multline}
 ...
 \left[\ln\left(1+\lambda_{\sigma}\xi_{s}\exp\left(-\frac{E}{T}\right)\right)\right]\,
\end{multline}
\begin{align}
    \label{partition:2}    
    \lambda_{\sigma}\xi_{s} = \exp{\frac{\mu_{\sigma}+\eta_{s}}{T}}\,,
\end{align}
where $p_{z}$ is the momentum parallel to the field axis and electric charge is $e\equiv q_{e^{+}}=-q_{e^{-}}$. The index $\sigma$ in \req{partition:1} is a sum over electron and positron states while $s$ is a sum over polarizations. The index $s$ refers to the spin polarization along the field axis: parallel $(s=+1)$ or anti-parallel $(s=-1)$ for both particle and antiparticle species. The quantum numbers of the energy eigenstate $E$ will be elaborated on in \rsec{sec:thermal}.

We are explicitly interesting in small asymmetries such as baryon excess over antibaryons, or one spin polarization over another. These are described by \req{partition:2} as the following two fugacities:
\begin{itemize}
...
\begin{multline}
 ...
 \ln{{\cal Z}_{e^{+}e^{-}}}=\frac{2{\cal B}V}{(2\pi)^{2}}\sum_{\sigma,s}^{\pm1}\sum_{n=0}^{\infty}\sum_{k=1}^{\infty}\int_{-\infty}^{+\infty}{\rm d}p_{z}\\
 \frac{(-1)^{k+1}}{k}\exp\left({k\frac{\sigma\mu+s\eta-{\tilde m}_{\sigma,s}\varepsilon^{n}_{\sigma,s}}{T}}\right)\,,
\end{multline}
\begin{align}
 \label{bapprox} 
 \sigma\mu+s\eta-{\tilde m}_{\sigma,s}\varepsilon_{\sigma,s}^{n}<0\,,
\end{align}
...
\begin{multline}
    ...
    \ln{{\cal Z}_{e^{+}e^{-}}}=\frac{2{\cal B}V}{(2\pi)^{2}}\sum_{n=0}^{\infty}\sum_{k=1}^{\infty}\int_{-\infty}^{+\infty}{\rm d}p_{z}\frac{(-1)^{k+1}}{k}\\
...
\begin{multline}
    ...
    \ln{{\cal Z}_{e^{+}e^{-}}}=\frac{2{\cal B}V}{(2\pi)^{2}}\sum_{s'}^{\pm1}\sum_{n=0}^{\infty}\sum_{k=1}^{\infty}\int_{-\infty}^{+\infty}{\rm d}p_{z}\frac{(-1)^{k+1}}{k}\\
    \left[2\cosh\left(k\frac{\mu-s'\eta}{T}\right)\right]\exp\left(-k\frac{{\tilde m}_{s'}\varepsilon_{s'}^{n}}{T}\right)
\end{multline}
with dimensionless energies redefined in terms of the polarization mass
\begin{gather}
...
\begin{multline}
 \label{boltzmann}
 \ln{\cal Z}_{e^{+}e^{-}}\simeq\frac{T^{3}V}{\pi^{2}}\sum_{s'}^{\pm1}\left[\cosh{\frac{\mu-s'\eta}{T}}\right]\\
...
\begin{align}
 \label{spin}
 \ln{\cal Z}_{\rm S}=\frac{T^{3}V}{\pi^{2}}\sum_{s'}^{\pm1}\left[\cosh{\frac{\mu-s'\eta}{T}}\right]\left(x_{s'}^{2}K_{2}(x_{s'})\right)\,,
\end{align}
\begin{multline}
 \label{spinorbit}
 \ln{\cal Z}_{\rm SO}=\frac{T^{3}V}{\pi^{2}}\sum_{s'}^{\pm}\left[\cosh{\frac{\mu-s'\eta}{T}}\right]\\
...
\begin{multline}
 \label{chem}
 n_{p}\frac{\pi^{2}}{T^{3}}=\sum_{s'}^{\pm1}\sinh\left(\frac{\mu-s'\eta}{T}\right)\\
 \left(x_{s'}^{2}K_{2}(x_{s'})+\frac{b_{0}}{2}x_{s'}K_{1}(x_{s'})+\frac{b_{0}^{2}}{12}K_{0}(x_{s'})\right)\,.
\end{multline}
\req{chem} is fully determined by the right-hand-side expression if the spin fugacity is set to unity $\eta=0$ implying no external bias to the number of polarizations (both parallel and anti-parallel) except as a consequence of the difference in energy eigenvalues. In practice, the latter two terms in \req{chem} are negligible to chemical potential in the bounds of the primordial $e^{+}e^{-}$ plasma considered and only becomes relevant for extreme (see \rf{fig:chemicalpotential}) magnetic field strengths well outside our scope.
...
\end{align}
If instead $\eta=0$ indicating zero spin asymmetry, we arrive at
\begin{multline}
    \label{simpchem:2}
    \sinh\frac{\mu}{T}=n_{p}\frac{\pi^{2}}{T^{3}}\\
    \left[\sum_{s'}^{\pm1}\left(x_{s'}^{2}K_{2}(x_{s'})+\frac{b_{0}}{2}x_{s'}K_{1}(x_{s'})+\frac{b_{0}^{2}}{12}K_{0}(x_{s'})\right)\right]^{-1}\,.
\end{multline}
...
In \rf{fig:chemicalpotential} we plot the chemical potential $\mu/T$ in \req{simpchem:1} and \req{simpchem:2} which characterizes the importance of the charged lepton asymmetry as a function of temperature. Since the baryon (and thus charged lepton) asymmetry remains fixed, the suppression of $\mu/T$ at high temperatures indicates a large pair density which is seen explicitly in \rf{fig:densityratio}. The black line corresponds to the $b_{0}=0$ and $\eta=0$ case. 
...
\begin{align}
 \label{g2magplus}
 {\mathfrak M}_{+}&=\frac{e^{2}}{\pi^{2}}\frac{T^{2}}{m_{e}^{2}}\cosh{\frac{\mu-\eta}{T}}\left[\frac{1}{2}x_{+}K_{1}(x_{+})+\frac{b_{0}}{6}K_{0}(x_{+})\right]\,,\\
\begin{split}  
 \label{g2magminus}
 -{\mathfrak M}_{-}&=\frac{e^{2}}{\pi^{2}}\frac{T^{2}}{m_{e}^{2}}\cosh{\frac{\mu+\eta}{T}}\\
...
\begin{figure}[ht]
 ...
 \caption{The magnetization $\mathfrak M$ as a function of $g$-factor plotted for several temperatures with magnetic scale $b_{0}=10^{-3}$ and spin fugacity $\xi=1$.}
 \label{fig:gfac} 
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{multline}
    \label{arbg:1}
    {\mathfrak M}_{\pm}=\frac{e^{2}}{\pi^{2}}\frac{T^{2}}{m_{e}^{2}}\cosh{\frac{\mu\mp\eta}{T}}\\
