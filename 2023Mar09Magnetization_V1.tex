...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Cosmic expansion considerations}
There is another natural scale for the magnetic field when considering the implications of FLRW expansion of the $e^{\pm}$ fluid. 
...
\begin{align}
  \label{PartFunc} \ln(\mathcal{Z})=\sum_{\alpha}\ln\left(1+e^{-\beta(E-\eta_{e})}\right)\,,
\end{align}
...
\begin{align}
\ln\mathcal{Z}_{tot}=\frac{2eBV}{(2\pi)^2}\sum_{n=0}^\infty\int^\infty_{0} \!\!dp_z\bigg[\ln\left(1+e^{-\beta(E_{n}^\pm-\eta_{e})}\right)+\ln\left(1+e^{-\beta(E_{n}^\pm+\eta_{e})}\right)\bigg],
\end{align}
where $\eta_{e}$ is the electron chemical potential, the positron chemical potential is the negative of the electron's, and energy $E_{n}^\pm$ can be written as
\begin{align}
...
\begin{align}
  ...
  &=\frac{2eBV}{(2\pi)^2}\sum_{n=0}^\infty\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k}\bigg[e^{k\beta\eta_{e}}+e^{-k\beta\eta_{e}}\bigg]\int_0^\infty dp_z\,e^{-k\beta E_n^\pm}\notag\\
  &=\frac{2eBTV}{(2\pi)^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^2}\bigg[2\cosh{(k\beta\eta_{e})}\bigg]\sum_{n=0}^\infty W^\pm_1(n,k),
\end{align}
...
\end{align}
where $R$ is the error remainder which is defined by integrals over Bernoulli polynomials. Using the asymptotic properties of Bessel function $K_{i}(x)$ as $x$ becomes large, we can simplify the above expression to obtain
\begin{align}
...
where we define 
\begin{align}
  \label{FreePart}&\ln\mathcal{Z}_{free}=\frac{2T^3V}{(2\pi)^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^4}\cosh{\left(\frac{k\eta_{e}}{T}\right)}\left[\left(\frac{k\tilde{m}_\pm}{T}\right)^2K_2\left(\frac{k\tilde{m}_\pm}{T}\right)\right]\,,\\
  \label{MagPart}&\ln\mathcal{Z}_B=\frac{4eBTV}{(2\pi)^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^2}\cosh{\left(\frac{k\eta_{e}}{T}\right)}\bigg[\frac{k\tilde{m}_\pm}{2T}K_1\left(\frac{k\tilde m_\pm}{T}\right)+\frac{k^2eB}{12T^2}K_0\left(\frac{k\tilde m_\pm}{T}\right)\bigg]\,,\\
  \label{ErrorPart}&\ln\mathcal{Z}_R=\frac{4eBTV}{(2\pi)^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^2}\cosh{\left(\frac{k\eta_{e}}{T}\right)}R\,.
\end{align}
...
\begin{align}
  \label{PartTotalNR} \ln\left(\mathcal{Z}_{tot}\right)\vert_{NR}&=\frac{2eBV}{(2\pi)^{2}}\sum_{n=0}^{\infty}\sum_{k=1}^{\infty}\frac{(-1)^{k+1}}{k}\left[2\cosh(k\beta\eta_{e}^{+})+2\cosh(k\beta\eta_{e}^{-})\right]\int_{0}^{\infty}dp_{z}e^{-k\beta\epsilon_{n}}\,,\\
...
Another approach to the non-relativistic electron-positron plasma is to consider the partition function obtained in \req{FreePart}-\req{ErrorPart} in the Boltzmann approximation and assuming the error remainder $R$ is small and can be neglected. In this case, we have
\begin{align}
\ln\mathcal{Z}_{tot}=&\frac{T^3V}{(2\pi)^2}\left[2\cosh(\eta_{e}/T)\right]\left(\frac{\tilde m_\pm}{T}\right)^{\!\!2} K_2(\tilde m_\pm/T)\notag\\
&+\frac{2eBTV}{(2\pi)^2}\left[2\cosh(\eta_{e}/T)\right]\left[\frac{1}{2}\left(\frac{\tilde m_\pm}{T}\right)K_1(\tilde m_\pm/T)+\frac{1}{12}\left(\frac{eB}{T^2}\right)K_0(\tilde m_\pm/T)\right]
\end{align}
It can be written as
\begin{align}\label{lnZ}
\ln\mathcal{Z}_{tot}=\frac{T^3V}{(2\pi)^2}\left[2\cosh(\eta_{e}/T)\right]\left\{\left(\frac{\tilde m_\pm}{T}\right)^{\!\!2} K_2(\tilde m_\pm/T)+\frac{eB}{T^2} \left[\left(\frac{\tilde m_\pm}{T}\right)K_1(\tilde m_\pm/T)+\frac{1}{6}\left(\frac{eB}{T^2}\right)K_0(\tilde m_\pm/T)\right]
\right\}
\end{align}
...
&=\frac{2T^3}{(2\pi)^2}\sinh{(\eta_{e}/T)}\left[\left(\frac{\tilde{m}_\pm}{T}\right)^2K_2(\tilde m_\pm/T)\right]+\frac{4eBT}{(2\pi)^2}\sinh{(\eta_{e}/T)}\bigg[\frac{\tilde{m}_\pm}{2T}K_1(\tilde m_\pm/T)+\frac{eB}{12T^2}K_0(\tilde m_\pm/T)\bigg]
\end{align}
Using the charge neutrality, we have
\begin{align}
n_p&=\left(n_e-n_{\bar e}\right)\notag\\
&=\frac{2T^3}{(2\pi)^2}\sinh{(\eta_{e}/T)}\left[\left(\frac{\tilde{m}_\pm}{T}\right)^2K_2(\tilde m_\pm/T)\right]
+\frac{4eBT}{(2\pi)^2}\sinh{(\eta_{e}/T)}\bigg[\frac{\tilde{m}_\pm}{2T}K_1(\tilde m_\pm/T)+\frac{eB}{12T^2}K_0(\tilde m_\pm/T)\bigg]
\end{align}
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In Fig {\ref{Chemical_fig}}, we plot the  chemical potential as a function of temperature $T$. It shows that the chemical potential is not sensitive to the magnetic field because the small value of $b_0=10^{-3}\sim10^{-8}$ and can be neglected in Eq.(\ref{ChemicalPotential}). 
...
\left(\frac{M}{B}\right)=\frac{4\pi\alpha}{(2\pi)^2b_0}\left[2\cosh(\eta_{e}/T)\right]\left\{\left[1-\frac{(1\pm g/2)}{2}\left(1+\frac{b^2_0}{6x^2_\pm}\right)\right]x_\pm K_1(x_\pm)+\left[\frac{1}{3}-\frac{(1\pm g/2)}{2}\right]b_0K_0(x_\pm)\right\}.
\end{align}
...
\begin{itemize}
 ...
 \left(\frac{M}{B}\right)&=-\frac{8\pi\alpha}{(2\pi)^2}\left(\frac{b_0}{6x}K_1(x_\pm)+\frac{2}{3}K_0(x_\pm)\right)\sqrt{1+\left[\frac{{(2\pi)^2n_p}/{(2T^3)}}{\left[x_\pm^2K_2(x_\pm)+b_0x_\pm K_1(x_\pm)+\frac{b^2_0}{6}K_0(x_\pm)\right]}\right]^2}
   \end{align}
Substituting the magnetic field $b_0$ and proton density $n_p/T^3$  we can solve the magnetization and chemical potential numerically. In Fig {\ref{Case1_fig}}, we plot the  magnetization $M/B$ as a function of temperature $T$. It shows for the case1 the magnetization is not sensitive to the magnetic field, this is because the small value of $b_0=10^{-3}\sim10^{-8}$ and can be neglected in Eq.(\ref{chemical_001}) and Eq.(\ref{Magnetization_001}).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[t]
%\begin{center}
\centering
\includegraphics[width=0.75\linewidth]{Magnetization_case1.jpg}
\caption{The chemical potential $\eta_{e}/T$ (upper) and magnetization $M/B$ (lower) as a function of temperature $20<T<60$keV  for the case 1. The chemical potential and magnetization are not sensitive to the magnetic field, because the small value of $b_0=10^{-3}\sim10^{-8}$ can be neglected in Eq.(\ref{chemical_001}) and Eq.(\ref{Magnetization_001}). }
\label{Case1_fig} 
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\
