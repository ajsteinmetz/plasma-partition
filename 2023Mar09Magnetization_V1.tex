...
\AuthorNames{Johann Rafelski, Cheng Tao Yang, and Andrew Steinmetz}
...
%\begin{center}
\centering
\includegraphics[width=0.75\linewidth]{NewDensity_cm3.jpg}
...
\noindent {\color{red}ANDREW: How much cosmology do we want here? I assume putting in at least the FLRW metric here such that $a(t)$ is defined is a minimal requirement for this section.} 
{\color{blue}
Considering the spatially flat Friedman-Roberston-Walker(FRW) universe, the spacetime metric in  rectangular coordinates can be written as
\begin{align}
ds^2=dt^2-a^2(t)\left[ dx^2+dy^2+dz^2\right].
\end{align}
which characterized by the scale factor $a(t)$ of a spatially homogeneous Universe. The Universe dynamics can be characterized by the quantity: the Hubble parameter $H$ as follows
\begin{align}
    H^2\equiv\left(\frac{\dot a}{a}\right)^2=\frac{8\pi G_N}{3}\rho_{tot}
\end{align}
where $G_N$ is the Newtonian constant of gravitation and $\rho_{tot}$ is the total energy density in early universe.
}
   

There is another natural scale for the magnetic field when considering the implications of FLRW expansion of the $e^{\pm}$ fluid. As the universe expands, different terms in the energies and thus partition function evolve as a function of the scale factor $a(t)$ which arises in the FLRW metric. We can consider the expansion to be an adiabatic process which result in a smooth shifting of the relevant dynamical quantities. From the conservation of magnetic flux through a co-moving surface, the magnetic field under expansion starting at some initial time $t_{0}$ is given by
\begin{alignat}{1}
...
\end{align}
where $\eta_{e}$ is the electron chemical potential, $\lambda_s=e^{\eta_s/T}$ is the spin chemical potential%and the positron chemical potential is the negative of the electron's 
...
\begin{align}
  ...
  &=\frac{2eBV}{(2\pi)^2}\sum_{n=0}^\infty\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k}\bigg[e^{k\beta\eta_{e}}+e^{-k\beta\eta_{e}}\bigg]\left(e^{\eta_s/T}\int_0^\infty dp_z\,e^{-k\beta E_n^+}+e^{-\eta_s/T}\int_0^\infty dp_z\,e^{-k\beta E_n^-}\right)\notag\\
  &=\frac{2eBTV}{(2\pi)^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^2}\bigg[2\cosh{(k\beta\eta_{e})}\bigg]\left(e^{\eta_s/T}\sum_{n=0}^\infty W^+_1(n,k)+e^{-\eta_s/T}\sum_{n=0}^\infty W^-_1(n,k)\right),
\end{align}
...
where we define 
\begin{align}
  \label{FreePart}&\ln\mathcal{Z}_{free}=\frac{T^3V}{2\pi^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^4}\left[2\cosh{\left(\frac{k\eta_{e}}{T}\right)}\right]\left[e^{k\eta_s/T}x_+^2K_2\left(x_+\right)+e^{-k\eta_s/T}x_-^2K_2\left(x_-\right)\right]\,,\\
  \label{MagPart}&\ln\mathcal{Z}_B=\frac{eBTV}{2\pi^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^2}\left[2\cosh{\left(\frac{k\eta_{e}}{T}\right)}\right]\bigg\{e^{k\eta_s/T}\,\bigg[\frac{x_\pm}{2}K_1\left(x_\pm\right)+\frac{k^2b_0}{12}K_0\left(x_\pm\right)\bigg]\notag\\
  &\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad+e^{-k\eta_s/T}\,\bigg[\frac{x_\pm}{2}K_1\left(x_\pm\right)+\frac{k^2b_0}{12}K_0\left(x_\pm\right)\bigg]\bigg\}\,,\\
\end{align}
and
\begin{align}
  \label{ErrorPart}&\ln\mathcal{Z}_R=\frac{eBTV}{2\pi^2}\sum^{\infty}_{k=1}\frac{(-1)^{k+1}}{k^2}\left[2\cosh{\left(\frac{k\eta_{e}}{T}\right)}\right]\left(e^{k\eta/T}\,R+e^{-k\eta/T}\,R\right)\,.
\end{align}
